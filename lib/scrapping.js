...
function anyMatches(value, refs) {
    ...
    return refs.some(target => {
        log("Info", normalizedValue, normalizeText(target))
        return normalizedValue.startsWith(normalizeText(target))
    });
}
...
function clickButton(testId, failSilently = false) {
    ...
    log("Info", `Button [${testId}]`, "Clicking button");
    button.click();
}